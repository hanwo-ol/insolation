# modules/dataset.py
# No Sobel , 1 pw using dataset.py
import torch
...
from modules_1pw_v2.sobel import sobel_edges  # Import sobel_edges

print("1 pw dataset used")
...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(5):
            ...
            if self.transform:
                ...
                if i == 4:
                    ...
                    # pre_power 1 with lag
                    pw = (img - img_for_pw).abs() ** 2
                    max_val = pw.max()
                    # Min-Max Normalization: [0, 4] -> [-1, 1]
                    pw = (pw / (max_val + 1e-09)) * 2.0 - 1.0  
                    
                    input_image.append(pw)
        ...
        # 타겟 이미지 및 파일명 로드
        target_img_path = self.image_files[sequence_start['index'] + 5 + self.lp]  # +1 제거, self.lp 사용
