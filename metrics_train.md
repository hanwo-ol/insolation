...
*   **Per Epoch Calculation:** The script collects all $MSE_{b,i}$ values into the `epoch\_mses` list and calculates the mean using `np.nanmean` (to handle potential NaNs if metric calculation failed).   

    $$MSE_{\text{epoch}} = \text{Mean}( \{ MSE_{b,i} \mid \forall {b, i} \} ) \approx \frac{1}{N_{\text{valid_images}}} \sum_{b=1}^{N_{\text{batches}}} \sum_{i=1}^{B} MSE_{b,i}$$
